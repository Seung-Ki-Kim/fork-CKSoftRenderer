...
namespace CK {
	...
	FORCEINLINE static constexpr bool EqualsInTolerance(float InFloat1, float InFloat2,
                                                        float InTolerance = SMALL_NUMBER) {
	...
	FORCEINLINE static constexpr T Clamp(const T X, const T Min, const T Max) {
        ...
        static FORCEINLINE constexpr void GetSinCosRad(float& OutSin, float& OutCos, float InRadian) {
            ...
            float p = ((((-2.6051615e-07f * y2 + 2.4760495e-05f) * y2 - 0.0013888378f) * y2 + 0.041666638f) * y2 - 0.5f)
                * y2 + 1.0f;
            OutCos = sign * p;
        }
