...
namespace CK {
    ...
    public:
        ...
        FORCEINLINE explicit constexpr Matrix4x4(const Vector4& InCol0, const Vector4& InCol1, const Vector4& InCol2,
                                                 const Vector4& InCol3) { Cols = {InCol0, InCol1, InCol2, InCol3}; }
...
FORCEINLINE constexpr Matrix4x4 Matrix4x4::operator*(const Matrix4x4& InMatrix) const {
        ...
        return Matrix4x4(
            Vector4(tpMat[0].Dot(InMatrix[0]), tpMat[1].Dot(InMatrix[0]), tpMat[2].Dot(InMatrix[0]),
                    tpMat[3].Dot(InMatrix[0])),
            Vector4(tpMat[0].Dot(InMatrix[1]), tpMat[1].Dot(InMatrix[1]), tpMat[2].Dot(InMatrix[1]),
                    tpMat[3].Dot(InMatrix[1])),
            Vector4(tpMat[0].Dot(InMatrix[2]), tpMat[1].Dot(InMatrix[2]), tpMat[2].Dot(InMatrix[2]),
                    tpMat[3].Dot(InMatrix[2])),
            Vector4(tpMat[0].Dot(InMatrix[3]), tpMat[1].Dot(InMatrix[3]), tpMat[2].Dot(InMatrix[3]),
                    tpMat[3].Dot(InMatrix[3]))
        );
    }
